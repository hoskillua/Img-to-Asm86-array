...
# This pallete is the default 256 colors in the TasmMasm Extension for VS code, Note that the original asm86 only supported the first 16 colors
...
# How to set up Krita for pixel art(5 mins) httpswww.youtube.comwatchv=aaRzNTCanIQ
# Nice Pixel art tutorials playlist (each video is 5~15 mins) httpswww.youtube.complaylistlist=PLmac3HPrav-9UWt-ahViIZxpyQxJ2wPSH
...
# Split the array into multiple lines cause it will be too long for one line for the tasmmasm.
for i in range(len(C))
    if i % 40 == 39
        StrC = StrC + str(C[i]) +   + n DB 
    else
        StrC = StrC + str(C[i]) + , 
